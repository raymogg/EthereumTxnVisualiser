...
export default class NodeInfo extends Component {
  ...
  render() {
    return this.state.selectedNode ? this.renderSelectedNode() : NodeInfo.renderNoneSelected()
  }
  ...
  renderSelectedNode() {
    return (
      <div className='selected-node'>
        <h4>
          <a
            href={`http://etherscan.io/address/${this.state.selectedNode.id}`}
            style={{ color: 'white' }}
            target='_blank'>
        ...
        <div className='row'>
          ...
          <div>
            ({this.state.currency}){this.state.netValue}
          </div>
        </div>
      </div>
    )
